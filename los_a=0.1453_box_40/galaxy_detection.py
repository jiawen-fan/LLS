...
def Box(Lbox, OmegaM, h100, aexp, halo, xg, yg, zg, vx, vy, vz, pos, e3):
    ...
    if e3[0] > 0:
        planePoint_x = np.array([[40, 0, 0],[80, 0, 0],[120, 0, 0]])
        planePoint_x_inverse = np.array([0,0,0])
    else:
        planePoint_x = np.array([[0, 0, 0],[-40, 0, 0],[-80, 0, 0]])
        planePoint_x_inverse = np.array([40,0,0])
    if e3[1] > 0:
        planePoint_y = np.array([[0, 40, 0],[0, 80, 0],[0, 120, 0]])
        planePoint_y_inverse = np.array([0,0,0])
    else:
        planePoint_y = np.array([[0, 0, 0],[0, -40, 0],[0, -80, 0]])
        planePoint_y_inverse = np.array([0,40,0])
    if e3[2] > 0:
        planePoint_z = np.array([[0, 0, 40],[0, 0, 80],[0, 0, 120]])
        planePoint_z_inverse = np.array([0,0,0])
    else:
        planePoint_z = np.array([[0, 0, 0],[0, 0, -40],[0, 0, -80]])
        planePoint_z_inverse = np.array([0,0,40])
    ...
    los_start_pts_inverse = np.where(np.logical_and(e3>0, intercept_pts_inverse == 0), 40, intercept_pts_inverse)
    distance_inverse = (-1)*np.sqrt(np.sum((intercept_pts_inverse-pos)**2,axis=1))

    distance = np.sqrt(np.sum((intercept_pts-pos)**2,axis=1))
    new_los_start_pts = np.vstack((pos,intercept_pts[distance<135]%40))
    new_los_start_pts = np.where(np.logical_and(e3<0, new_los_start_pts == 0), 40, new_los_start_pts)
    ...
    new_x = np.where(dx>20,new_x-40,new_x)
    new_x = np.where(dx<-20,new_x+40,new_x)
    new_y = np.where(dy>20,new_y-40,new_y)
    new_y = np.where(dy<-20,new_y+40,new_y)
    new_z = np.where(dz>20,new_z-40,new_z)
    new_z = np.where(dz<-20,new_z+40,new_z)
    ...
    distance_all = np.concatenate((np.insert(distance[distance<135],0,0),distance_inverse))[:, np.newaxis]
    w += distance_all
    ...
    # select galaxies
    sel1 = np.logical_and(w>5,w<95) # within first 100 cMpc/h
